...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_UNCOND ("Starting OLSR on node " << m_mainAddress);
...
// \brief Processes an incoming %OLSR packet following \RFC{3626} specification.
void
RoutingProtocol::YRecvOlsr (Ptr< Packet> receivedPacket, Ipv4Address src)
{
	NS_LOG_UNCOND("recv olsr packet from"<<src);

  Ipv4Address senderIfaceAddr = src ;
  NS_LOG_UNCOND("senderIfaceAddr"<<src);
  NS_LOG_UNCOND("myIfaceAddr"<<m_mainAddress);
//  Ipv4Address receiverIfaceAddr = this->m_mainAddress;
  Ipv4Address receiverIfaceAddr = m_socketAddresses.begin()->second.GetLocal ();
//  NS_ASSERT (receiverIfaceAddr != Ipv4Address ());
  NS_LOG_UNCOND ("OLSR node " << m_mainAddress << " received a OLSR packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_LOG_UNCOND("if set");
//  NS_ASSERT (inetSourceAddr.GetPort () == OLSR_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket->Copy();
  double tmp = packet->GetSize();

  olsr::PacketHeader olsrPacketHeader;
  packet->RemoveHeader (olsrPacketHeader);
  NS_LOG_UNCOND("remove header");
  NS_ASSERT (olsrPacketHeader.GetPacketLength () >= olsrPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = olsrPacketHeader.GetPacketLength () - olsrPacketHeader.GetSerializedSize ();
//  NS_LOG_UNCOND("111");
  MessageList messages;
...
RoutingProtocol::RoutingTableComputation ()
{
  ...
  for (NeighborSet::const_iterator it = neighborSet.begin ();
    ...
    {
      NeighborTuple const &nb_tuple = *it;
//      NS_LOG_UNCOND (Simulator::Now().GetSeconds()<<this->m_mainAddress<<"Looking at neighbor tuple: " << nb_tuple);
                             ...
                             const MessageList &containedMessages)
{
  ...
  NS_LOG_UNCOND (Simulator::Now().GetSeconds()<<"OLSR node " << m_mainAddress << " sending a OLSR packet");
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      // hui diao
      NS_LOG_UNCOND("aksld"<<i->second.GetLocal()<<","<<bcast);
      this->sendcallback(packet,bcast, this->m_mainAddress);//, this->m_mainAddress);// mac layer interface
//      i->first->SendTo (packet, 0, InetSocketAddress (bcast, OLSR_PORT_NUMBER));
      pktovhd++;
      bitsovld+=packet->GetSize();
    }
}
...
RoutingProtocol::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
  ...
  this->sendcallback(p,header.GetDestination (), this->m_mainAddress);
//  NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<"has data packet, call back to spma");
...
//      this->sendcallback(p,header.GetDestination (), this->m_mainAddress);
//      NS_LOG_UNCOND("has data packet, call back to spma");
      return rtentry;
...
Ipv4Address RoutingProtocol::YRouteOutput(Ipv4Address mip, Ipv4Address dst)
{
	 NS_LOG_UNCOND (Simulator::Now().GetSeconds()<<"Yrouteoutput " << mip<<"-----"<<dst);
	 Ptr<Ipv4Route> rtentry;
	  RoutingTableEntry entry1, entry2;
	  Ipv4Address ip;
//	  bool found = false;
	  if (Lookup (dst, entry1) != 0)
	     {
	       ...
	       if (!foundSendEntry)
	         {
	           NS_FATAL_ERROR ("FindSendEntry failure");
	           return ip;//if not found ,return 102.102.102.102
	         }
	       uint32_t interfaceIdx = entry2.interface;
	       rtentry = Create<Ipv4Route> ();
	            ...
	            NS_LOG_DEBUG ("Found route to " << rtentry->GetDestination () << " via nh " << rtentry->GetGateway () << " with source addr " << rtentry->GetSource () << " and output dev " << rtentry->GetOutputDevice ());
//	            found = true;
	            return rtentry->GetGateway();//if found return gateway
...
Ipv4Address RoutingProtocol::YRouteInput(Ipv4Address dst,Ptr<const NetDevice> idev)
{

	 Ipv4Address ip;
	 NS_LOG_FUNCTION (this << " " << m_ipv4->GetObject<Node> ()->GetId () << " ------" << dst);
...
//	           NS_LOG_LOGIC ("Local delivery to " << dst);
//
//	           return m_mainAddress;// if I am dst, return m_ip;
//	         }
	   ...
	   if (Lookup (dst, entry1))
	     {
	       ...
	       NS_LOG_DEBUG ("Olsr node " << m_mainAddress
	                                  << ": RouteInput for dest=" << dst
	                                  << " --> nextHop=" << entry2.nextAddr
	                                  << " interface=" << entry2.interface);


	       return entry2.nextAddr;// foward ,return gateway ip
	     }
	   else
	     {
	           ...
	           return ip;//return 102.102.102.102 if not found
